...
RELEVANCE_SYSTEM_PROMPT = """Ты — редактор тематического Telegram-канала. Тебе нужно решить, подходит ли новость в указанный раздел канала, по описанию интересов читателя.

Будь строгим. Если новость не по теме раздела — отклоняй. Если на грани — отклоняй.

Отвечай РОВНО ОДНИМ СЛОВОМ: ДА или НЕТ. Никаких пояснений, никакого текста вокруг."""
...
def is_relevant(title: str, summary: str, category_title: str, interests: str) -> bool:
    ...
    user_msg = (
        f"РАЗДЕЛ: {category_title}\n"
        f"ИНТЕРЕСЫ ЧИТАТЕЛЯ: {interests}\n\n"
        f"НОВОСТЬ\n"
        f"Заголовок: {title}\n"
        f"Описание: {(summary or '')[:400]}\n\n"
        f"Подходит ли эта новость в этот раздел? Ответь одним словом: ДА или НЕТ."
    )
    try:
