...
SOURCES = {
    ...
    "hiphop": {
        ...
        "hashtag": "#хипхоп",
        "interests": (
            "ТОЛЬКО хип-хоп, рэп, R&B и трэп — русский и зарубежный. "
            "Подходит: новые треки/альбомы/EP/клипы рэперов и R&B-исполнителей, "
            "бифы и драмы между рэп-артистами, чарты именно рэпа, "
            "концерты и туры рэперов, лейблы и индустрия рэпа, "
            "фрешмены и новые имена в рэпе, женщины в рэпе. "
            "КАТЕГОРИЧЕСКИ НЕ подходит и должно отклоняться: "
            "поп-музыка и поп-исполнители (Demi Lovato, Меган Трейнор, Майкл Джексон, Бритни Спирс, Тейлор Свифт), "
            "кантри (Ella Langley, фестиваль Stagecoach), "
            "рок и инди (Foo Fighters, любые рок-группы), "
            "электронная танцевальная музыка без рэп-контекста, "
            "комедийные шоу и стендап (SNL и его варианты), "
            "мюзиклы, фолк, классика, "
            "кино- и теле-премьеры даже если про музыкантов, "
            "светские новости про знаменитостей не из рэп-сцены. "
            "Если в новости главный герой — НЕ рэпер и НЕ R&B-исполнитель, отклоняй."
        ),
        "feeds": [
